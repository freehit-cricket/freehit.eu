...
class Generator:
    ...
    # Directories to exclude from addon scanning
    EXCLUDE_DIRS = ['repo', 'zips', 'docs', '.git']
    ...
    def _create_zips(self,addon_id,version):
        ...
        addon_folder = os.path.join(self.OUT_DIR,addon_id)
        if not os.path.exists(addon_folder):
            os.makedirs(addon_folder)

        final_zip = os.path.join(self.OUT_DIR,addon_id,'{0}-{1}.zip'.format(addon_id, version))
...
# Remove any instances of pyc or pyo files
    ...
    def _generate_addons_file(self):
        ...
        for addon in addons:
            try:
                if (not os.path.isdir(addon) or addon in self.EXCLUDE_DIRS or addon.startswith('.')): continue
        ...
        self._save_file(addons_xml.encode('utf-8'), file=os.path.join(self.OUT_DIR,'addons.xml'), decode=True)

    def _generate_md5_file(self):
        try:
            m = hashlib.md5(open(os.path.join(self.OUT_DIR,'addons.xml')).read().encode('utf-8')).hexdigest()
            self._save_file(m, file=os.path.join(self.OUT_DIR,'addons.xml.md5'))
        except Exception as e:
