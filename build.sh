...
EXCLUDE_FILES="build.sh repo zips .git"

# Clean previous build
rm -rf "$REPO_DIR" "$ZIP_DIR"
mkdir -p "$REPO_DIR/$ADDON_ID" "$ZIP_DIR"

# Copy addon files (excluding build artifacts)
for item in *; do
    skip=false
    ...
    if [ "$skip" = false ]; then
        cp -r "$item" "$REPO_DIR/$ADDON_ID/$item"
    fi
done

# Create zip
cd "$REPO_DIR"
zip -r "../$ZIP_DIR/$ADDON_ID-1.0.0.zip" "$ADDON_ID"
cd ..
...
<addons>
    <addon id="plugin.video.freehit" name="Freehit" version="1.0.0" provider-name="freehit.eu">
        <requires>
            ...
            <import addon="inputstream.adaptive" version="20.3.0"/>
...
- Multi-source fallback support</news>
    ...
    <addon id="repository.freehit" name="Freehit Repository" version="1.0.0" provider-name="freehit.eu">
        <extension point="xbmc.addon.repository" name="Freehit Repository">
            <info compressed="false">https://raw.githubusercontent.com/freehit-cricket/freehit.eu/main/repo/addons.xml</info>
            <checksum>https://raw.githubusercontent.com/freehit-cricket/freehit.eu/main/repo/addons.xml.md5</checksum>
            <datadir zip="true">https://raw.githubusercontent.com/freehit-cricket/freehit.eu/main/repo/</datadir>
        </extension>
        <extension point="xbmc.addon.metadata">
...
echo "Build complete!"
echo "Zip file: $ZIP_DIR/$ADDON_ID-1.0.0.zip"
